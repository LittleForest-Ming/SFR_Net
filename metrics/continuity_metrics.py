...
def _prepare_inputs(
    ...
    if pred_tensor.shape != target_tensor.shape:
        raise ValueError(
            f'continuity pred shape {tuple(pred_tensor.shape)} does not match '
...
def continuity_score(pred: torch.Tensor | None, target: torch.Tensor | None, valid_mask: torch.Tensor | None = None, threshold: float = DEFAULT_THRESHOLD) -> float:
    return continuity_f1(pred, target, valid_mask=valid_mask, threshold=threshold)
...
def compute_continuity_metrics(
    ...
    """Compute continuity metrics on valid pixels only.

    Predictions and targets are thresholded at `threshold` for IoU/F1, while MAE is
    computed on the continuous values. If `valid_mask` is provided, only masked pixels
    ...
    if pred is None or target is None:
        return {
            ...
            'continuity_f1': 0.0,
            'continuity_iou': 0.0,
            'continuity_bce_like_score': 0.0,
        }

    mae = continuity_mae(pred, target, valid_mask=valid_mask)
    f1 = continuity_f1(pred, target, valid_mask=valid_mask, threshold=threshold)
    iou = continuity_iou(pred, target, valid_mask=valid_mask, threshold=threshold)
    bce_like = max(0.0, 1.0 - mae)

    return {
        'continuity_mae': mae,
        'continuity_f1': f1,
        'continuity_iou': iou,
        'continuity_bce_like_score': bce_like,
    }
